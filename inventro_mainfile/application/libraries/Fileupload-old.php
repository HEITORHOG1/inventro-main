...
class Fileupload
{
    ...
    public function do_resize($file_path = null, $width = null, $height = null) {
        $ci =& get_instance();
        $ci->load->library('MY_Image_lib'); // Load the custom library
        ...
        $ci->my_image_lib->initialize($config);
        if (!$ci->my_image_lib->resize()) {
            echo $ci->my_image_lib->display_errors(); // Show errors if any
        }
    }
