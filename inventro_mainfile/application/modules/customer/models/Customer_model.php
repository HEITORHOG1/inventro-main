...
class Customer_model extends CI_Model {
	...
	{ 
		return $this->db->select("customer_tbl.*,ledger_tbl.id as lid,ledger_tbl.transaction_id,ledger_tbl.amount,ledger_tbl.d_c")->from($this->table)->join('ledger_tbl','customer_tbl.customerid=ledger_tbl.ledger_id','Left')
			->where('customer_tbl.id',$id) 
			->get()
			->row();
	} 
...
private function get_cusledger_query()
    {
    ...
    public function get_cusledger()
    {
        $this->get_cusledger_query();
   ...
   public function count_filtercusledger()
	{
		$this->get_cusledger_query();
		$query = $this->db->get();
		return $query->num_rows();
	...
	public function count_allcusledger()
	{
		$this->db->select('*');
        $this->db->from('customer_tbl');
