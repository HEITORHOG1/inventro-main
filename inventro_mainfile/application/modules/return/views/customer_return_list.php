...
<div class="card card-primary card-outline">
    <div class="card-header">
        <h4><?php echo makeString(['customer_return_list']); ?>
            <small class="float-right">
                <a href="<?php echo base_url('return/returns/customer_return'); ?>" class="btn btn-primary"> 
                    <i class="ti-plus" aria-hidden="true"></i><?php echo makeString(['customer_return']); ?>
                </a>
            </small>
        </h4>
    </div>
                    ...
                    <tbody>
                        ...
                        if ($return_list) {
                            foreach ($return_list as $return) {
                              $customers= $this->db->select('*')->from('customer_tbl')->where('customerid',$return->customer_id)->get()->row();
                                $sl++;
                                ?>
                                <tr>
                                    <td><?php echo $sl; ?></td>
                                    <td>
                                        <a href="<?php echo base_url('invoice/invoice/single_invoice/' . $return->invoice_id); ?>">
                                            <?php echo html_escape($return->invoice_id); ?>
                                        </a>
                                    </td>
                                    <td>

                                        <?php echo html_escape($customers->name); ?>
                                    </td>
                                    <td>
                                    ...
                                    <td class="text-center">
                                        <a class="btn btn-success btn-sm" href="<?php echo base_url('return/returns/customer_return_details/' . $return->return_id); ?>">
