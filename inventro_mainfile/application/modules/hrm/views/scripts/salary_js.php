<script type="text/javascript">
 ...
 'use strict';
	function addSalary()
	{
	    ...
	    $("#dip").attr("action", "<?php echo base_url('hrm/salary/save_salary')?>");
	...
	function editSalary(salary_id)
	{
			...
			var submit_url = "<?php echo base_url('hrm/salary/edit_salary'); ?>";
			...
			$.ajax({
				...
				success: function(res) {
				    ...
				    $("#dip").attr("action", "<?php echo base_url('hrm/salary/update_salary')?>");
				    $('#salary_form').modal('show'); 
	...
	function deleteSalary(salary_id)
	{
		...
		if(confirm('Deseja excluir?')===false){
			...
			var submit_url = "<?php echo base_url('hrm/salary/delete_salary'); ?>";
			var dataString = "salary_id="+salary_id;
       aler(CSRF_TOKEN);
			$.ajax({
	...
	function salaryGenerate()
	{
		...
		if(confirm('Deseja pagar o salário?')===false){
			...
			var submit_url = "<?php echo base_url('hrm/salary/salary_generat'); ?>";
	...
	function paymentSalary(generate_id)
	{
			...
			var submit_url = "<?php echo base_url('hrm/salary/salary_paid'); ?>";
			...
			$.ajax({
				...
				success: function(res) {
					...
					if(data){
					    ...
					    $("#dip").attr("action", "<?php echo base_url('hrm/salary/save_paid_salary')?>");
